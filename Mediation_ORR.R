...
###################### organisational remote readiness questions #####################

# Levels of organisation being well-prepared 
df_2021_new$last_6_months_org_wellprepared_for_me_working_remotely <- ordered(df_2021_new$last_6_months_org_wellprepared_for_me_working_remotely, levels = c(
  "Strongly disagree",
  "Somewhat disagree",
...
# levels of organisational remote collaboration
df_2021_new$last_6_months_easily_collaborate_with_collegues_when_working_remotely <- ordered(df_2021_new$last_6_months_easily_collaborate_with_collegues_when_working_remotely, levels = c(
  "Strongly disagree",
  "Somewhat disagree",
  "Neither agree nor disagree",
  "Somewhat agree",
...
# levels of IT equipment 
df_2021_new$have_IT_equipment_improved_or_worsened_over_last_6_months <- ordered(df_2021_new$have_IT_equipment_improved_or_worsened_over_last_6_months, levels = c(
...
ORR_1 <- df_2021_new$last_6_months_org_wellprepared_for_me_working_remotely

# Create a new numeric vector using nested ifelse statements:
SurveyM1 <- ifelse(ORR_1 == "Strongly disagree", 1,
                   ifelse(ORR_1 == "Somewhat disagree", 2,
                          ifelse(ORR_1 == "Neither agree nor disagree", 3,
                                 ifelse(ORR_1 == "Somewhat agree", 4,
                                        ifelse(ORR_1 == "Strongly agree", 5, NA)))))


###  ORR_2 ###

ORR_2 <- df_2021_new$last_6_months_easily_collaborate_with_collegues_when_working_remotely

# Create a new numeric vector using nested ifelse statements:
SurveyM2 <- ifelse(ORR_2 == "Strongly disagree", 1,
                   ifelse(ORR_2 == "Somewhat disagree", 2,
                          ifelse(ORR_2 == "Neither agree nor disagree", 3,
                                 ifelse(ORR_2 == "Somewhat agree", 4,
                                        ifelse(ORR_2 == "Strongly agree", 5, NA)))))



###  ORR_3 ###

ORR_3 <- df_2021_new$have_IT_equipment_improved_or_worsened_over_last_6_months

# Create a new numeric vector using nested ifelse statements:
SurveyM3 <- ifelse(ORR_3 == "Significantly worsened", 1,
                   ifelse(ORR_3 == "Somewhat worsened", 2,
                          ifelse(ORR_3 == "Stayed about the same", 3,
                                 ifelse(ORR_3 == "Not a barrier for me", 3,
                                      ifelse(ORR_3 == "Somewhat improved", 4,
                                            ifelse(ORR_3 == "Significantly improved", 5, NA))))))
...
samples_ORR <- jags(data, inits=initList, params,
                model.file ="jags_orr_1.txt",
...
############# Alpha_m1##################
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$alpha_M1) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$alpha_M2) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$alpha_M3) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$beta_M1) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$beta_M2) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$beta_M3) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$gamma_M1) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$gamma_M2) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$gamma_M3) # find the function that makes the posterior density plot
...
############## gamma total #############
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$gamma_total) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$psy_M1) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$psy_M2) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$psy_M3) # find the function that makes the posterior density plot
...
post_fit_fn = logspline(samples_ORR$BUGSoutput$sims.list$tau_prime) # find the function that makes the posterior density plot
